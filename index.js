  ...
  if (awaitingDateRange) {
    ...
    try {
      ...
      if (apiResponse.status !== 200) {
        message.reply("Terjadi kesalahan saat mendapatkan rekap kehadiran.");
        awaitingClassSelection = false;
        return;
      }

      const pdfUrl = apiResponse.data.url;
      ...
      if (errorMsg == undefined) {
        message.reply("Terjadi kesalahan saat mengirim rekap kehadiran.");
      } else {
    ...
    if (userMessage.toLowerCase() === "iya") {
      try {
        ...
        if (response.status === 200) {
          ...
          message.reply("PDF berhasil dibagikan ke orang tua/wali siswa.");
        } else {
          message.reply("Gagal mengambil data nomor WA.");
        }
      } catch (error) {
        console.error("Error saat mengambil nomor WA:", error);
        message.reply("Terjadi kesalahan saat mengambil data nomor WA.");
      }
